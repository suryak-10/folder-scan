...
def build_folder_meta(folder: Path, base: Path) -> dict:
    """Build metadata dict for a single (non-target) folder."""
    stat = _stat_safe(folder)
    return {
        "name":         folder.name,
        "absolutePath": str(folder.resolve()),
        "relativePath": str(folder.relative_to(base)),
        "created":      stat["created"]  if stat else None,
        "modified":     stat["modified"] if stat else None,
    }


def collect_target_contents(
    target_dir: Path,
    base: Path,
) -> tuple[list[dict], list[dict]]:
    """
    Recursively walk *target_dir* and return (files_list, folders_list).
    *base* is used to compute relative paths.
    """
    files:   list[dict] = []
    folders: list[dict] = []
    ...
    try:
        for item in target_dir.rglob("*"):
            if item.is_file():
                files.append(build_file_meta(item, base))
                summary["files_collected"] += 1
            elif item.is_dir():
                folders.append(build_folder_meta(item, base))
    except PermissionError as exc:
        log.warning("Permission denied scanning target %s: %s", target_dir, exc)
    except OSError as exc:
        log.warning("OS error scanning target %s: %s", target_dir, exc)
...
def scan_upload_targets(sequence_dir: Path, base: Path) -> list[dict]:
    ...
    for target_dir in children:
        ...
        files, folders = collect_target_contents(target_dir, base)
        targets.append({
